...
function RouteComponent() {
    const {
        ...
        reset
    } = usePersistentForm<LoginType>("loginForm", {
        ...
        <div className="flex min-h-screen bg-white dark:bg-[#30334E] text-gray-700 dark:text-gray-200">
            <div className="w-full lg:w-[45%] p-8 lg:p-16 flex items-center">
                <div className="w-full max-w-md mx-auto space-y-5">
                    ...
                    <div>
                        <h1 className="text-4xl font-bold text-gray-700 dark:text-gray-100">Welcome back!</h1>
                        <p className="mt-3 text-lg text-gray-600 dark:text-gray-300">Great to see you again. Let's get you signed in.</p>
                    </div>
                    <AnimatePresence>
                        {error && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <Alert color="danger" title={error.message} variant="solid" />
                            </motion.div>
                        )}
                    </AnimatePresence>
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                />
                                {errors.email && <div className="my-2 text-sm text-red-500">{errors.email?.message}</div>}
                            </div>
                            ...
                            <div>
                                ...
                                {errors.password && <div className="my-2 text-sm text-red-500">{errors.password?.message}</div>}
                            </div>
                        </div>
