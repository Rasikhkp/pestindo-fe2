...
export const Route = createFileRoute('/_layout/inventory_/supplier')({
  component: RouteComponent,
})

const getSupplierColumns = (
  onDelete: (supplier: Supplier) => void,
): ColumnDef<Supplier, any>[] => [
    {
      id: 'select',
      accessorKey: 'id',
      header: ({ table }) => (
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuItem>
              <Link
                to="/inventory/supplier/$supplier_id"
                params={{
            ...
            <DropdownMenuItem>
              <Link
                to="/inventory/supplier/edit/$supplier_id"
                params={{
                  supplier_id: String(row.original.id),
                }}
                className="w-full"
...
function RouteComponent() {
  ...
  useEffect(() => {
    if (queryError) {
      getApiErrorMessage(queryError).then((errorMessage) =>
  ...
  const supplierColumns = useMemo(
    () => getSupplierColumns(handleSingleDelete),
    [],
  )
  ...
  return (
    <div>
      ...
      <div className="flex justify-between mt-2 mb-8 text-sm text-gray-700 dark:text-gray-400">
        ...
        <Link to="/inventory/supplier/create">
      ...
      <DataTable
        ...
        onRowSelectionChange={(value) =>
          setTableState((prev) => ({
            ...prev,
            rowSelection:
              typeof value === 'function' ? value(prev.rowSelection) : value,
          }))
        }
