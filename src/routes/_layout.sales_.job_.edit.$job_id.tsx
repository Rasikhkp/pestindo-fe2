...
export const Route = createFileRoute("/_layout/sales_/job_/edit/$job_id")({
    ...
    loader: async ({ params }) => {
        try {
            const { data } = await api().get("jobs/" + params.job_id).json<any>();
            return {
                jobData: {
                    jobType: data.type,
...
function RouteComponent() {
    ...
    const updateJobMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            navigate({ from: "/sales/job/edit/$job_id", to: "/sales/job" });
        },
    ...
    useEffect(() => {
        if (updateJobMutation.error) {
            getApiErrorMessage(updateJobMutation.error).then(errorMessage => setError(errorMessage));
        }
    ...
    return (
        <>
            ...
            <div className="mb-6">
                <h1 className="text-2xl font-semibold dark:text-gray-300">Edit Pekerjaan</h1>
                <p className="text-gray-700 dark:text-gray-400">Silakan edit data pekerjaan dengan lengkap dan benar.</p>
            </div>
            <JobForm
